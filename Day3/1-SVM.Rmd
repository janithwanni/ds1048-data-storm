...
## Model fitting
```{r}
svm_tune <- tune.svm(NEXT_MONTH_DEFAULT ~ ., 
                       data=subset(train_split,select=-c(Client_ID)),
                       type="C-classification",
                       kernel="radial",
                       gamma = 2^(-2:2), cost = 2^(-2:2),
                       validation.x = subset(validation_split,select=-c(Client_ID,NEXT_MONTH_DEFAULT)),
                       validation.y = validation_split$NEXT_MONTH_DEFAULT,
                       tunecontrol=tune.control(sampling="fix",error.fun = F1_Score))
...
cm <- table(predictions,test_split$NEXT_MONTH_DEFAULT)
